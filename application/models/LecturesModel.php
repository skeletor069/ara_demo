...
class LecturesModel extends CI_Model {
    ...
    function CreateNewPost(){
        ...
        $m1 = date("m",$startDate);
        $d1 = date("d",$startDate);
        $sql = "insert into lectures(title,description,year,month,day,address,city) values(?,?,?,?,?,?,?)";
    ...
    function updatePost(){
        ...
        $m1 = date("m",$startDate);
        $d1 = date("d",$startDate);
        $sql = "update lectures set title=?,description=?,year=?,month=?,day=?,address=?,city=? where id=?";
        $q = $this->db->query($sql,array(
            $this->input->post("title"),
            $this->input->post("description"),
            $y1,
    ...
    public function publishNews($id){
        $sql = "update lectures set published=true where id=".$id;
        $this->db->query($sql);
    }

    public function draftNews($id){
        $sql = "update lectures set published=false where id=".$id;
        $this->db->query($sql);
    }
    ...
    function removePost($id){
        ...
        foreach ($images as $image) {
            ...
            $q = "delete from lecture_images where lecture_id=? and image_id=?";
    ...
    function GetPublishedNews(){
        $sql = "select * from lectures where published=true order by year desc,month desc,day desc";
        $q = $this->db->query($sql);
        $data = [];
        foreach ($q->result() as $row){
            $data[] = array(
    ...
    function GetPublishedNewsByYear($year){
        $sql = "select * from lectures where published=true and year=? order by year desc,month desc,day desc";
    ...
    function GetPostImages($id){
        $sql = "select g.* from gallery g,lecture_images n where g.image_id=n.image_id and n.lecture_id=?";
        $q = $this->db->query($sql,array($id));
        $data = [];
    ...
    function UploadPostImage(){
        ...
        if($id != null){
            $sql = "insert into lecture_images(lecture_id, image_id) values(?,?)";
    ...
    function GetUniqueYears(){
        ...
        $sql = "select distinct year from lectures where published=true order by year desc";
        $q = $this->db->query($sql);
        $data = [];
